<x-guest-layout>
    <div class="max-w-md w-full mx-auto bg-white dark:bg-slate-800 p-8 rounded-[28px] shadow-[0_10px_40px_-10px_rgba(0,0,0,0.04)] border border-slate-100/50 dark:border-slate-700/50 transition-colors duration-300">
        ...
        <div class="text-center mb-8">
            <h2 class="text-2xl font-bold text-slate-800 dark:text-white tracking-tight transition-colors">Register</h2>
            <p class="text-sm text-slate-400 mt-1.5 transition-colors">Please fill in the details to register.</p>
        </div>
        ...
        <form method="POST" action="{{ route('register') }}">
            ...
            <div class="mb-4">
                <div class="relative flex items-center">
                    <span class="absolute left-4 text-slate-400 dark:text-slate-500">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-5 h-5"><path stroke-linecap="round" stroke-linejoin="round" d="M21.75 6.75v10.5a2.25 2.25 0 0 1-2.25 2.25h-15a2.25 2.25 0 0 1-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0 0 19.5 4.5h-15a2.25 2.25 0 0 0-2.25 2.25m19.5 0v.243a2.25 2.25 0 0 1-1.07 1.916l-7.5 4.615a2.25 2.25 0 0 1-2.36 0l-7.5-4.615m19.5 0A2.25 2.25 0 0 0 19.5 6.75" /></svg>
                    </span>
            ...
            <div class="mb-4">
                <div class="relative flex items-center">
                    <span class="absolute left-4 text-slate-400 dark:text-slate-500">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-5 h-5"><path stroke-linecap="round" stroke-linejoin="round" d="M16.5 10.5V6.75a4.5 4.5 0 1 0-9 0v3.75m-.75 11.25h10.5a2.25 2.25 0 0 0 2.25-2.25v-6.75a2.25 2.25 0 0 0-2.25-2.25H6.75a2.25 2.25 0 0 0-2.25 2.25v-6.75a2.25 2.25 0 0 0 2.25 2.25Z" /></svg>
                    </span>
            ...
            <div class="mb-6">
                <div class="relative flex items-center">
                    ...
                    <input id="password_confirmation" class="block w-full pl-12 pr-12 py-3.5 bg-slate-50 dark:bg-slate-900/50 border-transparent dark:border-slate-700 focus:border-slate-200 dark:focus:border-slate-600 focus:bg-white dark:focus:bg-slate-900 focus:ring-0 rounded-2xl text-sm text-slate-700 dark:text-slate-200 placeholder-slate-400 dark:placeholder-slate-500 transition-all duration-200" type="password" name="password_confirmation" placeholder="Confirm Password" required />
                </div>
            ...
            <div class="mb-4">
                <div class="relative flex items-center">
                    ...
                    <select id="role_id" name="role_id" class="block w-full pl-12 pr-4 py-3.5 bg-slate-50 dark:bg-slate-900/50 border-transparent dark:border-slate-700 focus:border-slate-200 dark:focus:border-slate-600 focus:bg-white dark:focus:bg-slate-900 focus:ring-0 rounded-2xl text-sm text-slate-500 dark:text-slate-300 transition-all duration-200" required>
                        <option value="" disabled selected class="bg-white dark:bg-slate-800 text-slate-500">Select Role/Position</option>
                        @foreach ($roles as $role)
                            <option value="{{ $role->id }}" class="text-slate-700 dark:text-slate-200 bg-white dark:bg-slate-800" {{ old('role_id') == $role->id ? 'selected' : '' }}>
                                {{ $role->name }}
                            </option>
                        @endforeach
                    </select>
